...
export default function ReceivedLetters() {
    ...
    const fetchLetters = async (user: User) => {
        ...
        try {
            ...
            const response = await fetch('/api/received-letters', {
                headers: {
                    'Authorization': `Bearer ${idToken}`
                }
            })
            if (!response.ok) {
                throw new Error('Failed to fetch received letters')
            }
            const fetchedLetters = await response.json()
            setLetters(fetchedLetters)
        } catch (err) {
            console.error('Error fetching received letters:', err)
            setError('Failed to fetch received letters. Please try again later.')
        } finally {
            setLoading(false)
        }
    }
    ...
    return (
        <div className="container mx-auto mt-8 p-4">
            <h1 className="text-3xl font-bold mb-4">Received Letters</h1>
            {letters.length === 0 ? (
                <p className="text-gray-600">You haven't received any letters yet.</p>
            ) : (
